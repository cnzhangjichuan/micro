...
func (s *Service) Difference(dst string, only []string, sVersion bool) error {
	...
	for _, f := range fs {
		...
		if err := s.processFile(f, dst, ps, tpx, fileMD5); err != nil {
			packet.Free(pack)
			return err
		}
	}
...
// processFile 处理文件
func (s *Service) processFile(f os.FileInfo, cur string, ps int, tpx string, fileMD5 map[string]string) error {
	if f.IsDir() {
		...
		for _, c := range fs {
			if err := s.processFile(c, dr, ps, tpx, fileMD5); err != nil {
				return err
			}
		}
		return nil
